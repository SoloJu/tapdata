...
public class SyncMetricCollector implements ISyncMetricCollector {
    ...
    private AtomicLong getCdcTotals(String type) {
        ...
        if (null == v) {
            synchronized (cdcTotals) {
                v = cdcTotals.get(type);
                if (null == v) {
                    v = new AtomicLong(0);
                    cdcTotals.put(type, v);
                }
            }
        }
        return v;
    ...
    private AtomicLong getCdcDelayTotals(Long type) {
        ...
        if (null == v) {
            synchronized (cdcDelayTotals) {
                v = cdcDelayTotals.get(type);
                if (null == v) {
                    v = new AtomicLong(0);
                    cdcDelayTotals.put(type, v);
                }
            }
        }
        return v;
    ...
    public synchronized String toString() {
        ...
        if (null != snapshotBeginTimes && null != snapshotCompletedTimes) {
            ...
            buf.append(", \"snapshotDuration\": \"").append(Duration.ofMillis(snapshotCompletedTimes - snapshotBeginTimes)).append("\"");
            buf.append(", \"snapshotQPS\": ").append(new BigDecimal(snapshotTotals.get())
                .divide(BigDecimal.valueOf((snapshotCompletedTimes - snapshotBeginTimes) / 1000.0), 2, RoundingMode.HALF_UP)
            );
        }
        if (null != cdcBeginTimes && null != cdcFirstEventTimes) {
            ...
            buf.append(", \"cdcDuration\": \"").append(Duration.ofMillis(cdcCompletedTimes - cdcBeginTimes)).append("\"");
            buf.append(", \"cdcQPS\": ").append(new BigDecimal(totals.get())
                .divide(BigDecimal.valueOf((cdcCompletedTimes - cdcBeginTimes) / 1000.0), 2, RoundingMode.HALF_UP)
            );
